...
impl Rectangle {
    pub fn with_coordinates((x1, y1): (usize, usize), (x2, y2): (usize, usize)) -> Self {
        Rectangle { x1, x2, y1, y2 }
    }

    pub fn with_size((x1, y1): (usize, usize), (width, height): (usize, usize)) -> Self {
        Rectangle::with_coordinates((x1, y1), (x1 + width, y1 + height))
    }

    pub fn from_horizontal_line((x1, y1): (usize, usize), length: usize, width: usize) -> Self {
        Rectangle::with_coordinates((x1, y1), (x1 + length, y1 + width))
    }

    pub fn from_vertical_line((x1, y1): (usize, usize), length: usize, width: usize) -> Self {
        Rectangle::with_coordinates((x1, y1), (x1 + width, y1 + length))
    }
